      ...
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-lg border border-(--color-secondary)/10">
        {sucesso ? (
          <div className="flex flex-col items-center text-center py-10">
            ...
            <p className="text-(--color-secondary)/60 mt-2">
              Você já pode adotar seu novo melhor amigo. Redirecionando para o
              login...
            </p>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleCadastro} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-4 w-full bg-(--color-primary) text-white font-bold py-4 rounded-xl hover:bg-(--color-primary)/90 transition-all shadow-md flex justify-center items-center disabled:opacity-70"
              >
                {loading ? 'Criando conta...' : 'Cadastrar'}
              </button>
            </form>

            <div className="mt-6 text-center text-sm font-medium text-(--color-secondary)/60">
              Já tem uma conta?{' '}
              <Link href="/login" className="text-(--color-primary) font-bold">
                Faça login aqui
              </Link>
            </div>
          </>
